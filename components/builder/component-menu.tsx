  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        ...
        aria-label={label}
        title={label}
        className="studio-block-plus"
      >
        <Plus size={13} />
      </DropdownMenuTrigger>
      <DropdownMenuContent
        className="studio-context-menu studio-insert-menu"
        side="right"
      >
        {componentCatalog.map((item) =>
          item.name === "Emoji" ? (
            <DropdownMenuSub key={item.name}>
              <DropdownMenuSubTrigger>Emoji</DropdownMenuSubTrigger>
              <DropdownMenuSubContent className="studio-context-menu">
                {emojiNames.map((icon) => (
                  <DropdownMenuItem
                    key={icon}
                    onClick={() => onInsert(`<Emoji icon="${icon}" />`)}
                  >
              ...
              <DropdownMenuSubContent className="studio-context-menu">
                {[1, 2, 3].map((level) => (
                  <DropdownMenuItem
                    key={level}
                    onClick={() =>
                      onInsert(`${"#".repeat(level)} Heading ${level}`)
                    }
                  >
