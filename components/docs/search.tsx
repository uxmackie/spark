...
export function DocSearch({ product, theme }: { product: string; theme: CSSProperties }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/search?product=${encodeURIComponent(product)}`, {signal:controller.signal}).then(response => { if (!response.ok) throw new Error('Search unavailable'); return response.json() }).then(data => { setIndex(data.pages); setStatus('') }).catch(error => { if (error.name !== 'AbortError') setStatus('Could not load search. Please try again.') })
    return () => controller.abort()
  }, [open, index, retry, product])
  const highlight = (text: string) => highlightParts(text, deferredQuery).map((part, i) => part.match ? <mark key={i} className="docs-search-highlight">{part.text}</mark> : part.text)
  ...
  const matches = useMemo(() => searchPages(index ?? [], deferredQuery, product), [index, deferredQuery, product])
  return <>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-xl" style={theme}>
        <DialogHeader><DialogTitle>Find your next idea</DialogTitle><DialogDescription>Search page titles and content in this product.</DialogDescription></DialogHeader>
        ...
        {status && <p role="status">{status}{status.startsWith('Could not') && <button type="button" onClick={() => setRetry(n => n + 1)}>Retry</button>}</p>}<div className="flex max-h-80 flex-col gap-1 overflow-y-auto">
