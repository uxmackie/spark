  ...
  return <>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-xl" style={theme}>
        ...
        <label className="docs-search-filter">Product<StyledSelect label="Search product" value={product} onValueChange={setProduct} options={[{value: '', label: 'All products'}, ...(products.length ? products : [{slug: pages[0]?.product ?? '', name: pages[0]?.product ?? 'Current product'}]).filter(p => p.slug).map(p => ({value: p.slug, label: p.name}))]} /></label>{status && <p role="status">{status}{status.startsWith('Could not') && <button type="button" onClick={() => setRetry(n => n + 1)}>Retry</button>}</p>}<div className="flex max-h-80 flex-col gap-1 overflow-y-auto">
