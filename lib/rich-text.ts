  ...
  return nodes
    .map((node) => {
      ...
      if (
        ...
        return `<a href="${escapeHtml(node.url!)}"${node.title ? ` title="${escapeHtml(node.title)}"` : ""}>${inner}</a>`;
      if (["u", "s", "del", "strong", "em"].includes(node.name ?? ""))
      ...
      const attribute = (name: string) => {
        ...
        const expression = (
          value as {
            data?: {
              estree?: {
                body?: { expression?: { type?: string; value?: unknown } }[];
              };
            };
          }
      ...
      const display = ["Icon", "Emoji"].includes(node.name ?? "")
        ? iconMarkup(attribute("icon") ?? "face-smile")
        : node.name === "Tooltip"
          ? inner
          : escapeHtml(raw);
...
export function domToMarkdown(root: Node): string {
  const wrap = (text: string, marker: string) =>
    text.replace(/^(\s*)([\s\S]*?)(\s*)$/, (_, before, content, after) =>
      content ? before + marker + content + marker + after : text,
    );
  function walk(node: Node): string {
    ...
    switch (element.tagName.toLowerCase()) {
      ...
      case "code": {
        ...
        return `${fence}${value.startsWith("`") ? " " : ""}${value}${value.endsWith("`") ? " " : ""}${fence}`;
      }
      case "br":
        return "<br />";
      case "a": {
        ...
        return /^(https?:|mailto:|\/|#)/i.test(href) && !/[\s\\]/.test(href)
          ? `[${text}](${href.replace(/\(/g, "%28").replace(/\)/g, "%29")}${element.title ? ` "${element.title.replace(/"/g, "&quot;")}"` : ""})`
          : text;
      }
      case "div":
      case "p":
