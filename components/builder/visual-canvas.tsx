...
export function VisualCanvas({ source, product, onChange, disabled, pages = [] }: { source: string; product?: Product; onChange: (source: string) => void; disabled: boolean; pages?: { title: string; slug: string }[] }) {
  ...
  return <div className="studio-blocks studio-direct-canvas" aria-label="Visual document editor">{blocks.map((block, index) => <div data-editor-offset={block.start} key={`${epoch}:${index}`} className={`studio-block ${over === index ? 'studio-drop-target' : ''} ${dragged === index ? 'studio-dragging' : ''}`} onDragOver={event => { if (!disabled && event.dataTransfer.types.includes('application/spark-block')) { event.preventDefault(); setOver(index) } }} onDrop={event => { if (disabled) return; const from = Number(event.dataTransfer.getData('application/spark-block')); if (event.dataTransfer.types.includes('application/spark-block') && Number.isInteger(from)) { event.preventDefault(); move(from, index) } }}>
  ...
  </div>)}<div className="studio-bottom-insert"><ComponentMenu disabled={disabled} label="Add a component" onInsert={snippet => onChange(insertBlock(source, snippet, blocks.length))} /><button type="button" disabled={disabled} onClick={() => { onChange(insertBlock(source, '{/* spark-empty */}', blocks.length)); requestAnimationFrame(() => { const editors = document.querySelectorAll<HTMLElement>('.studio-direct-canvas [data-rich-editor]'); editors[editors.length - 1]?.focus() }) }}>Write a paragraph · type / for commands</button></div>
